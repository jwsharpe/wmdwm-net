...
// ../.wrangler/tmp/pages-dfDRlw/functionsWorker-0.08431816612678578.mjs
var __defProp2 = Object.defineProperty;
var __name2 = /* @__PURE__ */ __name((target, value) => __defProp2(target, "name", { value, configurable: true }), "__name");
var FETCH_HEADERS = { "User-Agent": "formie/1.0" };
async function resolveAlbumUrl(input) {
...
__name(resolveAlbumUrl, "resolveAlbumUrl");
__name2(resolveAlbumUrl, "resolveAlbumUrl");
function parseInitData(html) {
  const re = /AF_initDataCallback\(\{[^}]*data:(.+?), sideChannel:/g;
  let best = null;
  for (const match2 of html.matchAll(re)) {
...
__name(parseInitData, "parseInitData");
__name2(parseInitData, "parseInitData");
function extractPhotos(initData, albumUrl) {
...
__name(extractPhotos, "extractPhotos");
__name2(extractPhotos, "extractPhotos");
...
__name(fetchAlbumPhotos, "fetchAlbumPhotos");
__name2(fetchAlbumPhotos, "fetchAlbumPhotos");
var KEY = "wmdwm-formie";
...
async function onRequestGet2({ request }) {
  ...
  const token = params.get("p");
  if (!token) {
    return new Response("Missing p parameter", { status: 400, headers: CORS2 });
  }
  let url;
  try {
    url = decodePhotoUrl(token);
  } catch {
    return new Response("Invalid token", { status: 400, headers: CORS2 });
  }
  ...
  try {
    ...
    if (!res.ok) {
      return new Response("Upstream error", { status: 502, headers: CORS2 });
    }
...
var routes = [
  {
    routePath: "/formie/api/album",
    mountPath: "/formie/api",
    method: "GET",
    middlewares: [],
...
// .wrangler/tmp/bundle-EoNE7Q/middleware-insertion-facade.js
...
// .wrangler/tmp/bundle-EoNE7Q/middleware-loader.entry.ts
...
//# sourceMappingURL=functionsWorker-0.08431816612678578.js.map
